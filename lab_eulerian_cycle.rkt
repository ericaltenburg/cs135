...
(define (degree relation vertex)
  (if (equal? '() relation)
      0
      (if (and (equal? (caar relation) vertex) (equal? (car (cdr (car relation))) vertex))
          (+ 2 (degree (cdr relation) vertex))
          (if (equal? (caar relation) vertex)
...
(define (mvHelper relation max)
  (if (equal? '() relation)
      max
      (if (> (caar relation) max)
          (mvHelper (cdr relation) (caar relation))
          (mvHelper (cdr relation) max))))
...
(define (has-eulerian-cycle? relation)
   (ecHelper relation (maxVertex relation)))

(define (ecHelper relation vertex)
  (if (equal? 0 vertex)
      #t
      (if (even? (degree relation vertex))
          (ecHelper relation (- vertex 1))
          #f)))
