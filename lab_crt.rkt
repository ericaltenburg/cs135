...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist)
      #t
      (if (Hexist (cadar tuplelist) (cdr tuplelist))
          (CRT-exists? (cdr tuplelist))
          #f)))
(define (Hexist m0 tuplelist)
  (if (null? tuplelist)
      #t
      (if (equal? (EuclidAlgo m0 (cadar tuplelist)) 1)
          (Hexist m0 (cdr tuplelist))
          #f)))
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist)
      0
      (+(*(car(car tuplelist))(/ m(cadar tuplelist))(mul_inv(/ m(cadar tuplelist))(cadar tuplelist))) (CRT-helper (cdr tuplelist)m))))
