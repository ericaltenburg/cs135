...
(define (composite  relationOuter relationInner)
  (if (null? relationOuter)
      '()
      (append(CHelper(car relationOuter)relationInner)
             (composite(cdr relationOuter)relationInner))))
(define(CHelper tuple inner)
  (if (null? inner)
      '()
      (if(equal?(cadar inner)(car tuple))
         (cons (list(caar inner)(cadr tuple)) (CHelper tuple(cdr inner)))
         (CHelper tuple (cdr inner)))))
...
(define (power relation k)
  (if (equal? k 0)
      '()
      (PHelper relation relation k)))
(define (PHelper relation power k)
  (if (equal? k 1)
      power
      (PHelper relation(composite relation power)(- k 1))))
...
(define (transitive-closure relation)
  (THelper relation relation(length relation)))
(define(THelper r rel len)
  (if (equal? len 0)
      '()
      (if (equal? len 1)
          rel
          (union rel(THelper r(power r len)(- len 1))))))
