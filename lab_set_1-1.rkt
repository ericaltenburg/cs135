...
(define (intersection listA listB)
  (if (equal? '() listA)
      ...
      (if (element? (car listA) listB)
          (make-set (cons (car listA) (intersection (cdr listA) (cdr listB))))
...
(define (cardinality lst)
  (if (equal? '() (make-set lst))
      0
      (+ 1 (cardinality (cdr (make-set lst))))))
...
(define (superset? setA setB)
  (and (subset? setB setA) (>= (cardinality setA) (cardinality setB))))
