...
(define (convert10 number base)
  (reverse (HConvert number base))
  )

(define (HConvert number base)
  (if (equal? 0 number)
      '()
      (cons (modulo number base) (HConvert (quotient number base) base))
      )
  )
...
(define (base10 oldbase numlist)
  (HBase oldbase (reverse numlist) 1)
  )
(define (HBase oldbase numlist current)
  (if (null? numlist)
      0
      (+ (* (car numlist) current) (HBase oldbase (cdr numlist) (* current oldbase)))
  )
  )
...
;; for some reason these examples had the wrong definition, saying it was (geometric-prog 1 10 5) so I changed it to geo-prog
...
(define (geo-prog cur_num ratio n)
  (if (equal? 0 n)
      '()
      (cons cur_num (geo-prog (* cur_num ratio) ratio (- n 1)))
      )
  )